...
1. An **initContainer** uses the plugin OCI image and copies:
   - `/plugin/oidc_provider/` → an `emptyDir` mounted at
     `/opt/CTFd/CTFd/plugins/oidc_provider` in the CTFd container, and
...
3. The RSA signing key lives on a small **PVC** (`OIDC_PROVIDER_JWK_FILE`) so
...
- Set a real `OIDC_PROVIDER_ISSUER` and a strong `SECRET_KEY` (use a `Secret`).
