...
Drop this directory into ``CTFd/plugins/`` (e.g. ``CTFd/plugins/oidc_provider``)
...
# The plugin's import/folder name, e.g. "oidc_provider". Used to build the
...
def load(app):
    ...
    try:
        ...
        app.logger.warning("OIDC IdP: could not pre-load signing key: %s", e)
    ...
    # (set OIDC_PROVIDER_APPS_FILE). No-op if the variable is unset.
    try:
        provision_from_yaml(app)
    except Exception as e:  # pragma: no cover - defensive
        app.logger.error("OIDC IdP: YAML provisioning failed: %s", e)
    ...
    # The admin management page (/admin/oidc) is surfaced under the admin
